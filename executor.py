    ...
    def warm_up_browser(self, url="https://olymptrade.com/platform"):
        ...
        for ctx in browser.contexts:
            for pg in ctx.pages:
                ...
                if 'doubleclick' in page_url or 'fls.' in page_url:
                    continue
                if '/platform' in page_url:
                    page = pg
                    break
            if page:
                break
        
        # Fallback: pick the first real (non-tracking) page
        if page is None:
            for ctx in browser.contexts:
                for pg in ctx.pages:
                    page_url = pg.url.lower()
                    if 'doubleclick' not in page_url and 'fls.' not in page_url:
                        page = pg
                        break
                if page:
                    break
        ...
        print("\033[92m[Browser] Ready. Trades will execute on the current page.\033[0m")
